    ...
    login: function () {
     ...
     // var query = 'client_id=' + AUTH_CLIENT_ID + '&redirect_uri=' + window.location + '&response_type=token&scope='
      ...
      if (window.cordova && window.device.platform !== 'browser') {
        var oAuthWindow = window.cordova.InAppBrowser.open('http://todosbackend.cristianfonolla.2dam.acacha.org:8080/oauth/authorize?' + query, '_blank', 'location=yes')
        var login = this
        ...
        console.log('http://todosbackend.cristianfonolla.2dam.acacha.org:8080/oauth/authorize?' + query)
        window.location.replace('http://todosbackend.cristianfonolla.2dam.acacha.org:8080/oauth/authorize?' + query)
      }
    },
    initlogout: function () {
